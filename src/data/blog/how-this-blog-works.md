...
文章保存在 `src/data/blog` 目录中，每篇文章都是一个 Markdown 文件。文件开头的区域负责保存标题、发布日期、摘要和标签：
...
Astro 在构建时读取所有公开文章，为每篇文章生成独立页面，同时更新首页、文章列表、RSS 和站点地图。

## 自动发布

代码推送到 GitHub 后，GitHub Actions 会执行构建并把生成结果发布到 GitHub Pages。以后发布新文章的基本流程只有三步：
